...
class FileSnippet {
  static readonly IMAGE_SIZE = { width: 300, height: 400 }
...
export class ImageUploadComponent implements OnInit {
  ...
  processFile(event: any) {
    ...
    if ((file = event.target.files[0]) && (file.type === 'image/png' ||
      ...
      img.onload = function () {
        if ((this.width > FileSnippet.IMAGE_SIZE.width) &&
          ...
          self.toastr.error(`Min width is ${FileSnippet.IMAGE_SIZE.width} and min height is ${FileSnippet.IMAGE_SIZE.height}`, 'Error!')
        }
      }
      img.src = URL.createObjectURL(file);
    } else {
      // handle error
      this.toastr.error('Unsupported File Type. Only JPEG, PNG, and JPG is allowed!', 'Error!')
    }
  }
